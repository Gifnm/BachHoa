...
public interface ProductPositionJPA extends JpaRepository<ProductPositioning, Integer> {
	/*
	 * 1 Lay danh sach vi tri theo mam Tham so: ma so cua hang, ma so ke, ma so mam
	 * Dung khi: + Can hien thi danh sach vi tri san pham theo mam trung bay + in
	 * tem gia theo mam
	 */
	...
	/*
	 * 2 Lay danh sach vi tri theo theo ke Tham so: ma so cua hang, ma so ke, ma so
	 * mam Dung khi: + in tem gia theo ke trung bay
	 */
...
// Lay danh sach vi tri theo ke
	// Sap xep tang dan theo vi tri (id) va mam (disPlaID)
	@Query("SELECT p FROM ProductPositioning p WHERE p.store.storeID = :storeID AND p.displayShelves.disSheID = :disSheID ORDER BY p.id ASC, p.displayPlatter.disPlaID  ASC")
